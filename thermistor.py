...
outfile = 1			# флаг вывода в файл
tts = 5				# интервал опроса, с

R25 = 10**4			# 10 кОм при 25C на термисторе, для B57861S0103
sv = 3.3			# напряжение питания, В
...
if outfile: outfile = open(f'./{datetime.datetime.now().strftime("thermistor_%Y%m%d_%H%M%S")}.log', 'w')
...
try:
	...
	while True:
		...
		if outfile: outfile.write('\t'.join([str(secs_left),	# время
							 					f'{v_adc:.3f}',	# U
							 					f'{rt25:.3f}',	# Rt/R25
							 					f'{t1:.1f}'		# T
		])+'\n')
